...
import { artSize, maxColumns } from "../src/core/size.ts";
...
test("exports the maximum supported cell count", () => { expect(maxColumns).toBe(4096); });
test("clamps oversized requests to 4096 Unicode columns", () => { expect(artSize(512, 512, 9999)).toEqual({ dotsWidth: 8192, dotsHeight: 8192, rows: 2048 }); });
