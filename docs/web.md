...
Maker polarity follows the surface when the theme or colour mode changes. Light monochrome/background-coloured output starts non-inverted. Light foreground-only colour uses the dark preview surface and therefore starts inverted. Dark mode starts inverted. The Invert checkbox remains manually adjustable afterward.

**Reset sliders** restores only Width, Contrast, Detail and Threshold bias to 96, 1.12, 0.34 and +0.015. It does not change colour mode, background/full-colour selections, dithering or polarity.
