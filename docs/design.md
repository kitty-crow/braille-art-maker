...
PNG is vectorised to path-only SVG, rasterised back to RGBA, resized to the Unicode dot grid, toned, dithered and packed into 2x4 Unicode cells.

Normal colour samples the source pixels represented by on-dots. Background colour separately samples off-dots. Full colour clusters the eight RGBA samples in each cell into up to two perceptual colour groups; cluster membership becomes the Unicode mask and the two means become foreground/background colours. Near-uniform cells use the same colour for both, producing a visually solid text cell.
