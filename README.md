...
The maker's **Copy embed div** output carries the generated result itself, so the original PNG does not need to be uploaded or hosted. The art payload is compacted before it is placed in the otherwise plain HTML fragment: each Unicode cell becomes its 8-bit dot mask, repeated masks and colour states are packed, the resulting binary is losslessly DEFLATE-compressed, and that compressed binary is encoded as base64url using the versioned `u2` codec.

Only the art payload is compressed/encoded. The surrounding `<div>`, `<template>`, stylesheet link, loader and API script references stay normal readable HTML. The encoder/decoder source also stays normal TypeScript in `src/embed/codec.ts`; the decoder is bundled into the published runtime.

The CDN runtime remains backwards-compatible with `u1` embeds generated by 0.4.5.
...
```ts
import { embedCodec, makeArt, packEmbed, unpackEmbed, taggedText, vectorStage } from "@kitty-crow/unicode-art-maker";
```
